...
const Main = () => {
    ...
    const handleCardClick = (promptText) => {
		setInput(promptText);
		fetchGeneratedText(promptText);
	};

    const fetchGeneratedText = (promptText) => {
		setLoading(true);
        ...
        .then(data => {
			setLoading(false);
			setResultData(data[0].generated_text); // Adjust based on your response structure
        })
        .catch(error => {
			setLoading(false);
			...
			<div className="main-container">
				...
				<div className="main-bottom">
					...
					<div className="bottom-info">
						<p>
							SysML may display inaccurate info, including about people, so
							double-check its responses. sysML.AI V1.00
						</p>
					</div>
				</div>
			</div>
